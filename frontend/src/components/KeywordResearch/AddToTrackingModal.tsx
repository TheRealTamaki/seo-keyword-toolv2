...
interface AddToTrackingModalProps {
  keywords: ResearchKeyword[];
  onClose: () => void;
  onSuccess: () => void;
}

const AddToTrackingModal: React.FC<AddToTrackingModalProps> = ({
  keywords,
  onClose,
  onSuccess,
}) => {
  ...
  const handleAddToTracking = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to add keywords to tracking');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="px-6 py-4 space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
              <p className="text-sm text-blue-900">
                Adding {keywords.length} keyword{keywords.length !== 1 ? 's' : ''} to rank tracking
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-8 bg-gray-50 rounded-md">
                ...
                <p className="text-xs text-gray-400">Please create a project first</p>
              </div>
            ) : (
              <>
                <div>
...
export default AddToTrackingModal;
